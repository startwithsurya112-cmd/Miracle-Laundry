...
import { Settings, Store, Receipt, CheckCircle, Save, Trash2, Upload, X, FileSpreadsheet, Download, UploadCloud, CheckCircle2, Smartphone, RefreshCw, QrCode, FileJson, Building2 } from 'lucide-react';

export const SettingsPage: React.FC = () => {
  const { showToast } = useToast();
  const { isSuperAdmin, selectedShop } = useAuth();
  ...
  return (
    <div className="space-y-6 pb-20 max-w-4xl mx-auto">
      ...
      {selectedShop && (
        <div className="px-4 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-50 via-white to-brand-50 dark:from-emerald-950/40 dark:via-slate-900 dark:to-brand-950/40 border border-emerald-200/80 dark:border-emerald-800/60 shadow-xs flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
            <span className="font-semibold text-slate-700 dark:text-slate-200">
              Active Branch: <strong className="text-emerald-700 dark:text-emerald-400">[{selectedShop.code}] {selectedShop.name}</strong> ({selectedShop.region})
            </span>
          </div>
